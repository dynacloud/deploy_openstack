...
function neutron_setup() {
  ...
  if [ "${NEUTRON_CREATE_NETWORKS}" == "True" ];then
    # Make our networks
    ${NEUTRON_NAME} net-create --provider:physical_network=ph-br-${NEUTRON_INTERFACE_BRIDGE} \
  ...
  # Configure OVS
  ovs-vsctl add-port br-${NEUTRON_INTERFACE_BRIDGE} ${NEUTRON_INTERFACE}
...
# Enable || Disable Neutron
NEUTRON_ENABLED=${NEUTRON_ENABLED:-"True"}

# Set the Interface
NEUTRON_INTERFACE_BRIDGE=${NEUTRON_INTERFACE_BRIDGE:-"ex"}
NEUTRON_INTERFACE=${NEUTRON_INTERFACE:-"eth1"}

# Set the Name of the Neutron Service
NEUTRON_NAME=${NEUTRON_NAME:-"neutron"}

# Enable || Disable Auto Create Networks
NEUTRON_CREATE_NETWORKS=${NEUTRON_CREATE_NETWORKS:-"False"}

# Set network name
NEUTRON_NETWORK_NAME=${NEUTRON_NETWORK_NAME:-"aioionet"}

# Enable || Disable LBaaS
LBAAS_ENABLED=${LBAAS_ENABLED:-"True"}

# Enable || Disable VPNaaS
VPNAAS_ENABLED=${VPNAAS_ENABLED:-"True"}

# Enable || Disable FWaaS
FWAAS_ENABLED=${FWAAS_ENABLED:-"True"}

# Set the default Run list
#RUN_LIST=${RUN_LIST:-"role[allinone],role[cinder-all]"}
RUN_LIST="role[allinone],role[single-network-node],role[heat-all],role[cinder-all]"

# Default Images
UBUNTU_IMAGE=${UBUNTU_IMAGE:-True}
FEDORA_IMAGE=${FEDORA_IMAGE:-False}
CIRROS_IMAGE=${CIRROS_IMAGE:-True}
